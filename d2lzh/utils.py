...
train_folder = '1_train_rgb' 
mask_folder = '2_train_mask'
test_folder = '3_test_rgb'
...
hand_colormap= [[0, 0, 0], [255, 255, 255]]
...
#-------------------------------------------------------------------------------------
def read_images( image_dir,size, index, image_list = None):
    
    """Read hand images."""
    
    global ROOT_DIR
    path = os.path.join(ROOT_DIR, 'data', image_dir)
    images = []
    ...
    for image_name in image_list[index:index+size]:
        images.append(image.imread(os.path.join(path,image_name)))
        
    print("照片总数为:" + str(len(images))) 
    print("读取完毕")
    return images, image_list

def read_features_and_labels(feature_dir, label_dir, size,index):
    
    features, image_list = read_images(feature_dir, size, index)
    labels, _ = read_images(image_dir = label_dir, image_list = image_list,index = index, size = size)
    
    return features, labels

    '''  
        for photo in photos[:batch_size]:
            features.append(image.imread(os.path.join(data_path, photo)))
            labels.append(image.imread(os.path.join(root, mask_folder,photo)))
    print("读取完毕")
    return features, labels  '''

# 手部数据集
class HandDataset(gdata.Dataset):
    
    rgb_mean = nd.array([0.485, 0.456, 0.406])
    rgb_std = nd.array([0.229, 0.224, 0.225])
    
    def __init__(self,feature_dir, label_dir, crop_size, colormap2label, size, index = 0):# 特征集，标签集 ，

        self.crop_size = crop_size
        data, labels = read_features_and_labels(feature_dir = feature_dir, label_dir = label_dir, size = size, index = index)
        self.data = [HandDataset.normalize_image(im) for im in self.filter(data)]
        self.labels = self.filter(labels)
        self.colormap2label = colormap2label
        print('read ' + str(len(self.data)) + ' examples')
    
    @classmethod
    def normalize_image(cls, data):
        return (data.astype('float32') / 255 - cls.rgb_mean) / cls.rgb_std
    ...
    def rand_crop(feature, label, height, width):
        """Random cropping for images of the hand Dataset."""
        feature, rect = image.random_crop(feature, (width, height))
        label = image.fixed_crop(label, *rect)
        return feature, label
    ...
    def label_indices(colormap, colormap2label):
        """Assign label indices for hand Dataset."""
        colormap = colormap.astype('int32')
        idx = ((colormap[:, :, 0] * 256 + colormap[:, :, 1]) * 256+ colormap[:, :, 2])
        return colormap2label[idx]
    
    # 继承dataset必须要重写的两个魔法函数
    def __getitem__(self, idx):
        data, labels = self.rand_crop(self.data[idx], self.labels[idx],*self.crop_size)
        
        return (data.transpose((2, 0, 1)),self.label_indices(labels, self.colormap2label))
